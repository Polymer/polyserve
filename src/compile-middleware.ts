  ...
  return transformResponse({
    ...
    transform(request: Request, response: Response, body: string): string {
      ...
      if (contentType === htmlMimeType) {
        ...
        transformed = compileScript(
            body, filePath, isComponentRequest, moduleResolution, options);
      } else {
        transformed = body;
      }
    ...
    if (transformingModule && !isInline) {
      ...
      try {
        compiled = jsTransform(js, {
          compile: options.transformES2015,
          minify: false,
          modulesToAmd: transformingModule,
          moduleResolution,
          filePath,
          isComponentRequest,
        });
...
function compileScript(
    ...
    moduleResolution: 'none'|'node',
    options: CompileOptions): string {
  return jsTransform(source, {
    compile: options.transformES2015,
    minify: false,
    modulesToAmd: options.transformModules && hasImportOrExport(source),
    moduleResolution,
    filePath,
    isComponentRequest,
  });
